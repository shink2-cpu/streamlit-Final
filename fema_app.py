...
"""The results of this project show that households with higher repair costs are much more likely to qualify for TSA assistance, indicating that the program generally targets those with more severe damage. The comparison between states also revealed significant differences in average repair amounts, suggesting that disaster impacts vary widely by location. Our statistical tests confirmed that these differences are meaningful and not due to random variation. Overall, the findings support the idea that TSA eligibility is strongly connected to the level of housing damage and that regional factors should be considered in FEMA’s disaster planning.
...
"""Using the sampled data, I trained both a Decision Tree and a Random Forest to predict TSA eligibility based on variables like income, repairAmount, residence type, and waterLevel. After comparing accuracy, precision, recall, and the confusion matrices, the Random Forest model performed better overall. Because it averages many trees, the Random Forest tends to generalize better to new data and is less likely to overfit compared to a single Decision Tree. For this reason, I would choose the Random Forest as the preferred model for predicting TSA eligibility.
...
"""For Part IV, I built an interactive Streamlit dashboard to visualize key patterns in FEMA’s disaster housing data. The app displays a histogram of repairAmount to show the overall distribution of housing repair costs and a boxplot of repairAmount by TSA eligibility to compare damage levels between eligible and non-eligible households. Each chart includes clear titles, axis labels, and short written insights that explain what the visual patterns suggest about the relationship between damage and assistance. The dashboard is published on Streamlit Cloud, and I included both the public link and a screenshot in this report."""